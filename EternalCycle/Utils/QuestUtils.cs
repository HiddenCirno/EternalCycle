...
namespace EternalCycleServer
{
    ...
    public static class QuestUtils
    {
        ...
        /// <summary>
        /// 把自定义数据里的单数 / 复数区域 ID 归一化进条件的 zoneIds。
        ///
        /// 注意：必须先清空。条件是先从 SPT 的原版模板 Clone 出来的，而原版模板自带 zoneIds
        ///   —— 例如 PlaceBeacon 模板带着 place_peacemaker_007_2_N2。
        ///   只追加不清空的话，我们的条件会连原版那个区域一起匹配上。
        ///
        /// 单数（zoneid）与复数（zoneids）两种写法都收，合并去重后统一写进 ZoneIds；
        /// 单数字段补上第一个值，保证两种读法都拿得到（幂等，无副作用）。
        /// </summary>
        ...
        public static void InitPlaceItemDataConditions(List<QuestCondition> conditions, PlaceItemData placeItemData, LoadModContext context)
        {
            ...
            // 区域统一走 ApplyZoneIds：先清掉模板残留，再合并单数/复数并去重
