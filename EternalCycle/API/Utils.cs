...
namespace EternalCycleServer
{
    ...
    public static class Utils
    {
        ...
        /// 源对象【在原始 JSON 里实际写了】的键集合（一般直接传模组 JSON 的 _props 节点）。
        /// 传了它就走「只覆盖模组写过的键」的严格语义；传 null 则退化为旧的「非 null 即覆盖」行为。
        /// </param>
        public static void CopyNonNullProperties(object source, object target, JsonObject writtenKeys = null)
        {
            ...
            // 键名统一大小写不敏感地比对：SPT 模型虽然都标了 [JsonPropertyName]，
            // 但属性名与键名偶有大小写差异，宽松一点更稳。
            ...
            foreach (var sourceProp in sourceType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                // ★ 判据必须是「这个键在原始 JSON 里存在吗」。
                //
                //   旧实现只判 value != null —— 这对值类型永远成立（int 的「没写」就是 0，
                //   装箱后并不等于 null），于是模组没写的字段会被 0 盖掉刚从原版克隆来的真实值。
                //   后果：Width/Height 变 0 → CalculateCellSize() 得 (0,0) →
                //   图标尺寸 0×0 → RenderTexture.GetTemporary(0,0) 抛 ArgumentException，
                //   物品在物品栏里彻底不可见；同一机制还会顺手把 Weight / StackMaxSize /
                //   MaxHpResource 等一并清零。
                //
                //   但也不能改成「跳过等于默认值的属性」—— 那会吞掉模组显式写的 0
                //   （RITC 的示例针包就写着 "Weight": 0，是有效取值）。
                //   所以只能回到 JSON 里查键。
        ...
        /// <summary>
        /// 取属性对应的 JSON 键名。SPT 的模型统一标了 [JsonPropertyName]，取不到才退回属性名。
        /// </summary>
        ...
        public static T ResolveJsonNode<T>(JsonNode node, JsonUtil jsonUtil)
        {
            ...
            var result = jsonUtil.Deserialize<T>(resultJson); // 返回处理后的 JsonNode

            // 顺手把「模组实际写了哪些 _props 键」挂到模板上。
            // 反序列化本身是有损的：默认值（int 的 0、bool 的 false）和「模组没写」长得一模一样，
            // 而覆盖阶段（CopyNonNullProperties）必须能把两者区分开，否则会把从原版克隆来的
            // Width / Height / Weight 等值类型字段统统清零。
